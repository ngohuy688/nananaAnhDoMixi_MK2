...
void drawCashDrawer() {
	...
	drawPosBlock(vec3(2.2, -0.4, 1.0), vec3(0.5, 0.15, 0.6));

	// --- 2. VẼ NGĂN KÉO DI ĐỘNG (Phần trượt ra) ---
	// Tạo ma trận gốc cho phần di động (Di chuyển theo trục X)
	mat4 drawer_ctm = Translate(2.2 + cashDrawerOpenAmount, -0.4, 1.0);
...
void drawPosScreen() {
	...
	// Chân đế màn hình 
	drawPosBlock(vec3(2.2, -0.25, 1.0), vec3(0.15, 0.15, 0.15));

	// Thân màn hình chính 
	drawPosBlock(vec3(2.2, -0.1, 1.0), vec3(0.05, 0.35, 0.5));
	...
	drawPosBlock(vec3(2.225, -0.1, 1.0), vec3(0.01, 0.3, 0.45));
}
...
void drawReceiptPrinter() {
	...
	drawPosBlock(vec3(2.2, -0.4, 1.5), vec3(0.3, 0.25, 0.25));
	...
	drawPosBlock(vec3(2.3, -0.27, 1.5), vec3(0.05, 0.01, 0.15));
}
void banthungan() {
	// 1. VẼ BÀN GỖ
	setMaterial(
	...
	drawWallBlock(vec3(2.2, -1.0, 0.5), vec3(0.8, 1.0, 2.7));

	// Mặt bàn (Mỏng hơn, rộng hơn thân một chút)
	drawWallBlock(vec3(2.2, -0.48, 0.5), vec3(0.9, 0.05, 2.7));
